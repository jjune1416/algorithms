...
struct container
{
    ...
    int value;
    int count;
    container(int in, int va, int c):index(in),value(va),count(c){};
};
...
int HowManyLIS(int index)
{
    ...
    ret = 0;
    for(int i = index + 1 ;i <= length ; i++)
        if(arr[i] > arr[index] && LIS(index) == LIS(i) + 1)
...
std::string KLIS(int index, int lisLen, int k)
{
    ...
    std::vector<container> counts;
    for(int i = index + 1; i <= length ; i++)
        if(arr[i] > arr[index] && LIS(i) == lisLen - 1)
            counts.push_back(container(i, arr[i], HowManyLIS(i)));
